...
async function fetchCommentsForPost() {
  ...
  try {
    ...
    // This will handle the server errors that could occur due to some problems in the code
    // we wrote on the server side.
  ...
  // This try catch will handle the cases where the user goes offline while he tries to
  // load the comments by pressing "Load Comments" button.
  // This will now show an alert saying "Getting comments failed!" instead of
  // No internet dinosaur page.
}
...
async function saveComment(event) {
  ...
  try {
    ...
    // There's a ok filed in const response. (response.ok)
    // This will be true if we get a 200 success response.
    // Else if we get a 500 error code, this will be false.
    // This will handle the server errors that could occur due to some problems in the code
    // we wrote on the server side.
  } catch (error) {
    alert("Could not send request - maybe try again later!");
  }
  // We can catch errors by using try catch when we deal with async await code.
  // This try catch will handle the cases where the user goes offline while filling the form and
  // the form isn't submitted after the submit button is pressed because he's offline.
  // In the browser dev tools, we can find a network tab. In there we can simulate the scenario where
  // the user goes offline. (It's on the dropdown menu that says "No throttling")
  // This will now show an alert saying "Could not send request - maybe try again later!" instead of
  // No internet dinosaur page.
}
